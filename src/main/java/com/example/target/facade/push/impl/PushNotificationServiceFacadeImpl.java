package com.example.target.facade.push.impl;

import com.sfl.nms.api.internal.facade.notification.common.impl.AbstractNotificationServiceFacadeImpl;
import com.sfl.nms.api.internal.facade.notification.push.PushNotificationServiceFacade;
...
import com.sfl.nms.core.api.internal.model.push.PushNotificationModel;
import com.sfl.nms.core.api.internal.model.push.PushNotificationPropertyModel;
import com.sfl.nms.core.api.internal.model.push.PushNotificationRecipientModel;
...
import com.sfl.nms.services.notification.event.push.StartPushNotificationSubscriptionRequestProcessingEvent;
...
public class PushNotificationServiceFacadeImpl extends AbstractNotificationServiceFacadeImpl implements PushNotificationServiceFacade {
    ...
    public ResultResponseModel<CreatePushNotificationResponse> createPushNotifications(@Nonnull final CreatePushNotificationRequest request) {
        ...
        final List<PushNotificationModel> pushNotificationModels = pushNotifications.stream().map(pushNotification -> createPushNotificationModel(pushNotification)).collect(Collectors.toCollection(ArrayList::new));
    ...
    /* Utility methods */
    private UserDevice getOrCreateUserDevice(final Long userId, final String userDeviceUuId, final DeviceOperatingSystemType operatingSystemType) {
        final UserDevice userDevice;
        final boolean deviceExists = userDeviceService.checkIfUserDeviceExistsForUuId(userId, userDeviceUuId);
        if (deviceExists) {
            userDevice = userDeviceService.getUserDeviceByUuId(userId, userDeviceUuId);
        } else {
            userDevice = userDeviceService.createUserDevice(userId, new UserDeviceDto(userDeviceUuId, operatingSystemType));
        }
        return userDevice;
    }
    ...
    private PushNotificationRecipientModel createPushNotificationRecipientModel(final PushNotificationRecipient recipient) {
